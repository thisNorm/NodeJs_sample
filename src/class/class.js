...
var CookingRobot = /** @class */ (function (_super) {
    ...
    // Constructor(생성자)
    function CookingRobot(name, availableMenus, model) {
        var _this = _super.call(this, name, model) || this;
        _this.availableMenus = availableMenus;
        return _this;
    }
    // Method(행동)
    CookingRobot.prototype.performTask = function () {
        console.log("".concat(this.getName, " is Cooking according to the menus ").concat(this.availableMenus.join(", "), "."));
    };
    return CookingRobot;
}(Robot));
